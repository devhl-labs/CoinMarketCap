...
namespace devhl.CoinMarketCap.Test.Api
{
    ...
    /// </summary>
    public sealed class KeyApiTests : ApiTestsBase
    {
        private readonly IKeyApi _instance;

        public KeyApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IKeyApi>();
        }
        ...
        public async Task GetV1KeyInfoAsyncTest()
        {
            var response = await _instance.GetV1KeyInfoAsync();
            Assert.IsType<AccountInfoResponseModel>(response);
        }
    }
}
